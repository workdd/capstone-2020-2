...
class Audio(Data):
    def __init__(self, platform, video_id, url):
    ...
    def load_audio(self, filetype):
        files = []
        for i in os.listdir(f'./{filetype}/{self.platform}/'):
        ...
        if filetype == "video":
            ...
            for filename in files:
                arr.append(AudioFileClip(f"audio/{self.platform}/{filename}"))  # 음성 파일 로드
    ...
    def save_graph(self, AVG_20=0.221829165):  # AVG_20 = 유튜브 하이라이트 영상 20개에 대한 평균
        ...
        plt.savefig(path + f"{self.video_id}.png")
